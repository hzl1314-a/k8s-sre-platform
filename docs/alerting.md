...
**演练执行时间：** 2026-09-13 20:39:19（Asia/Shanghai，cp 节点时区）
**脚本版本：** 采样版 `alert-drill.sh`（投递时刻用计数器采样，见 §4.0）

| 时刻 | 事件 | 距故障 | 证据 |
|---|---|---|---|
| T+0s（20:39:19） | `kubectl scale deploy/frontend --replicas=0` | — | `alert-drill.log` |
| T+13s（20:39:32） | 告警状态变为 Pending | **+13s** | `alert-drill.log` |
| T+73s（20:40:32） | 告警状态变为 **Firing** | **+73s** | `alert-drill.log` |
| T+73s（20:40:32） | Alertmanager 收到告警 | **+73s** | `alert-drill.log` |
| ≈T+78s（20:40:36） | **邮件 / 钉钉投递** | **≈+78s** | 截图 14 / 15（只到分钟） |
| T+193s（20:42:32） | 执行恢复（`--replicas=2`） | +193s | `alert-drill.log` |
| T+224s（20:43:03） | 告警 Resolved | **+224s**（距恢复 31s） | `alert-drill.log` |
| ≈T+230s（20:43:08） | 恢复通知（邮件 + 钉钉各一条） | ≈+230s | 截图 16（20:43） |

> ⚠️ 标「≈」的两组是**收件人界面的分钟级时间 + `groupWait: 5s` 推算**出来的，
> **不是实测秒数**。跑一次 §4.0 的命令换成实测值后再定稿——记录纪律见 §4.1 末尾。
...
```
 时间基准 : 2026-09-13T20:39:19+08:00
 采样文件 : alert-drill.log.samples
      时刻    通道   距故障 事件
      20:40:36 email    +77      告警投递
      20:43:08 email    +229     恢复投递
      20:40:36 webhook  +77      告警投递
      20:43:08 webhook  +229     恢复投递
```
